...
class Simulator:
    def __init__(self, config_path="config/config.yaml", output_file=sys.stdout, seed=0):
        ...
        self.network = Network(
            logger=self.logger,
            rng=self.rng,
            min_delay=self.config.get("simulation", {}).get("min_delay", 0.1),
            max_delay=self.config.get("simulation", {}).get("max_delay", 0.5),
            drop_prob=self.config.get("network", {}).get("drop_prob", 0.0),
            dup_prob=self.config.get("network", {}).get("dup_prob", 0.0)
        )
        
        # Initialize Nodes
        num_nodes = self.config.get("simulation", {}).get("num_nodes", 4)
        ...
        # Get gossip_k from config
        gossip_k = self.config.get("network", {}).get("gossip_k", 7)
